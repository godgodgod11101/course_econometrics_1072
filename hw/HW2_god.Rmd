...
coeftest(model1, vcov. = vcovHC, type = "HC1") -> model1_robust
model1_robust
# 用robust standard error比較保險
...
<ul>
  ...
  <li>假設其他情況不變，隨著媽媽的$age$增加，會使$children$平均的增加量愈來愈小（有邊際遞減的趨勢），如圖。</li>
</ul>

```{r}

fertil2 %>%
  summarize( maxage=max(age), minage=min(age) ) %>%
  {seq(from=.$minage, to=.$maxage, by=0.05)} -> X_age
# 用summarize函式先算min、max，再用數列存成X軸資料

tidy(model1)$estimate -> model1_coe
names(model1_coe) <- tidy(model1)$term
Y_ChildrenForAgeEffect <- model1_coe["age"]*X_age + model1_coe["agesq"]*X_age^2
plot(x=X_age, y=Y_ChildrenForAgeEffect)
# 用tidy將model1改成tibble物件，取其估計係數，並將之命名；再定義Y軸資料（為beta_2*age + beta_3*age^2）
...
ivreg(children~educ+age+agesq | frsthalf+age+agesq, data=fertil2) -> TSLSmodel1
coeftest(TSLSmodel1, vcov. = vcovHC, type = "HC1") -> TSLSmodel1_robust

stargazer(model1_robust, TSLSmodel1_robust,
...
coeftest(model3, vcov. = vcovHC, type = "HC1") -> model3_robust

ivreg(children~educ+age+agesq+electric+tv+bicycle | age+agesq+electric+tv+bicycle+frsthalf, data=fertil2) -> TSLSmodel3
coeftest(TSLSmodel3, vcov. = vcovHC, type = "HC1") -> TSLSmodel3_robust

stargazer(model3_robust, TSLSmodel3_robust,
          column.labels = c("OLS","TSLS"),
          type = "html",
          align=T          
          )
...
```{r ans15,results='asis'}

summary(TSLSmodel3, vcov = vcovHC, type = "HC1", diagnostics = TRUE, df=Inf) -> TSLSmodel3_test
# 若要作IV檢定，不可用coeftest函式

stargazer(TSLSmodel3_test$diagnostics, type = "html", align = T)


```

在給定5%的顯著水準下，檢定的結果是不顯著的，表示無足夠證據顯示必須使用TSLS來做迴歸模型。
...
不行，因為這裡的工具變數個數等於解釋變數個數，為exactly-identified，所以表示卡方分配的自由度為0，無法作檢定。
...
可以，因為這裡的工具變數個數大於解釋變數個數，為over-identified，自由度不會等於0，可以作檢定。<br>
在給定5%的顯著水準下作Sargan test，檢定結果顯示為不顯著，表示無足夠的證據顯示工具變數與誤差項有關，即工具變數過了排除條件檢定這關。
...
效應解釋變數使用替代變數的效應模型：
$$y_i = \beta^* \tilde{x_i} - \beta^* \epsilon_{i} + \gamma w_i + y_{-x,-w,i}$$

<br>
給定$\tilde{x_i}$和$w_i$條件下的迴歸模型：
$$E(y_i | \tilde{x}_i,w_i) = E(\beta^* \tilde{x_i} | \tilde{x}_i,w_i) - E(\beta^* \epsilon_{i} | \tilde{x}_i,w_i) + E(\gamma w_i | \tilde{x}_i,w_i) + E(y_{-x,-w,i} | \tilde{x}_i,w_i)$$
$$= \beta^* \tilde{x_i} + \gamma w_i - \beta^* E(\epsilon_{i} | \tilde{x}_i,w_i) + E(y_{-x,-w,i} | \tilde{x}_i)$$
</br>
